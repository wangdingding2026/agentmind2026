...
- `get_settings`, `feishu_get_config`, and `embedding_status` are read-side control views. They should move behind a settings/status query service when the control plane read model is consolidated.
...
1. Connector/settings read-side service for low-risk read views.
