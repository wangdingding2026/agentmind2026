...
const fixtures = {
  ...
  '/embedding/status': { has_local_model: false },
  '/tasks?limit=50': {
  ...
  '/connectors': { connectors: [{ id: 'a2', name: 'Agent Two', type: 'cli', tags: ['search'] }] },
...
async function main() {
  ...
  const context = {
    ...
    fetch: async (url, options = {}) => {
      ...
      if (url === '/v1/agents/scan') return { json: async () => ({ ok: true }) };
  ...
  assert(document.getElementById('agents-tbody').innerHTML.includes('data-action="edit-agent-tags"'));
  await context.testAgent('a1');
  await context.restartAgent('a1');
  await context.toggleAgent('a1');
  await context.editAgentTags('a1');
  document.getElementById('new-agent-id').value = 'new_agent';
  document.getElementById('new-agent-name').value = 'New Agent';
  document.getElementById('new-agent-cmd').value = 'echo ok';
  ...
  assert(document.getElementById('audit-tbody').innerHTML.includes('Embedding 配置'));
